...
class _EnTestViewState extends State<EnTestView> {
  ...
  // Color _buttonColor = white;

  @override
  void initState() {
    super.initState();
    question = []; // Ensure the list is initialized before usage.
    for (var q in widget.list) {
      int i = Random().nextInt(3); // Randomly select correct answer index
      question.add(
        Question(
          uz: q.uz,
          en: q.en,
  ...
  Color getButtonColor(int index) {
    if (hasAnswered) {
      if (selectedAnswerIndex == index) {
        ...
        return question[currentQuestionIndex].answerIndex == index
            ? Colors.green.shade600
            : Colors.red.shade600;
      } else if (question[currentQuestionIndex].answerIndex == index) {
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final option = question[currentQuestionIndex].options[index];
                  final isCorrect =
                      index == question[currentQuestionIndex].answerIndex;
